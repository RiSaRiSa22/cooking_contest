...
<task type="auto">
  <name>Task 1: Data layer — competitionStore + useCompetition + compress + usePhotoUpload</name>
  <files>
    src/store/competitionStore.ts
    src/hooks/useCompetition.ts
    src/hooks/usePhotoUpload.ts
  ...
  <action>
    ...
    **2. Create `src/hooks/useCompetition.ts`**
    ...
    - Admin reads from `dishes` table directly (NOT dishes_public) — the `anon_read_dishes` RLS policy allows this
    - Set data into competitionStore
    - Subscribe to Supabase Realtime: `competitions` table UPDATE events filtered by `id=eq.${competitionId}` — on change, call `updatePhase(payload.new.phase)`
    ...
    **4. Create `src/hooks/usePhotoUpload.ts`**
    ...
    - `uploadPhotos(files: File[], dishId: string, isExtra?: boolean): Promise<string[]>` — compresses each file via `compressImage()`, uploads to Storage bucket `dish-photos` at path `${dishId}/${crypto.randomUUID()}.jpg`, returns array of public URLs
    ...
    NOTE: Storage anon INSERT policy does not exist yet — a migration will be added in Plan 02-02. This hook will work once that policy is in place. For now, just implement the code.
  </action>
  <verify>
    Run `npx tsc --noEmit` — zero errors. All 4 files exist and export their public APIs.
  </verify>
  <done>
...
<task type="auto">
  <name>Task 2: AdminScreen shell + PhaseBanner + router wiring</name>
  <files>
    src/screens/Admin/AdminScreen.tsx
    src/components/competition/PhaseBanner.tsx
    src/router.tsx
  ...
  <action>
    **1. Create `src/components/competition/PhaseBanner.tsx`**
    ...
    - `preparation`: amber/gold background, text "Preparazione in corso" with emoji "👨‍🍳"
    - `voting`: ember/red background, text "Votazione in corso" with emoji "🗳️"
    - `finished`: sage/green background, text "Gara conclusa" with emoji "🏆"
    ...
    **2. Create `src/screens/Admin/AdminScreen.tsx`**
    ...
    Tab content: render placeholder `<div>` for each tab with "Tab {name} — coming soon" text. Plans 02-02 and 02-03 will replace these placeholders.
  ...
  <verify>
    Run `npm run build` — zero errors. Navigate to `/#/admin/TESTCODE` (with a valid session) — AdminScreen renders with header, PhaseBanner, 4 tab buttons, and tab content area.
  </verify>
  <done>
...
3. All 7 new files exist at their specified paths
