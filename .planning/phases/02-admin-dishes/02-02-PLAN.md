...
must_haves:
  truths:
    ...
    - "Admin can delete any dish with confirmation — DB record and Storage blobs both removed"
...
Purpose: This is the core CRUD feature of Phase 02 — admin must be able to create, edit, and delete dishes with photos. Requires both backend (Edge Functions) and frontend (components + tab) in a single vertical slice.
  ...
  <action>
    ...
    **1. Create `supabase/functions/dish-write/index.ts`**
    ...
    4. If participant.role !== 'admin':
       ...
       - **isExtra carve-out (DISH-06):** if `isExtra === true` AND `phase === 'voting'` AND the participant owns the dish (dish.participant_id === participantId), ALLOW the write (participants can add extra photos during voting)
       - Otherwise, if phase !== 'preparation': return 403 "Piatti modificabili solo in fase preparazione"
    ...
    > **Scoping note (DISH-02 / DISH-03):** The EF enforces participant dish-write rules server-side (one dish per participant, own-dish-only edits, preparation-phase gate). The participant-facing UI for adding/editing their own dish is intentionally deferred to Phase 3, where the VoterScreen "Il mio piatto" tab (VOTR-04 / Plan 03-02) will provide it. This is NOT a gap — the backend is ready; the UI depends on the Voter screen that doesn't exist yet.
    ...
    **2. Create `supabase/functions/dish-delete/index.ts`**
    ...
    Deletion pipeline (ORDER MATTERS — Pitfall 6 from research):
...
<task type="auto">
  <name>Task 2: DishCard + PhotoGrid + AddDishModal + DishesTab wiring</name>
  <files>
    src/components/dishes/DishCard.tsx
    src/components/dishes/PhotoGrid.tsx
    src/screens/Admin/modals/AddDishModal.tsx
  ...
  <action>
    ...
    **2. Create `src/components/dishes/DishCard.tsx`**
    ...
    - Body: dish name (font-display, text-lg, font-bold), chef name (text-sm, text-gray-500, prefixed "👨‍🍳"), ingredients preview (first 80 chars + "..." if longer)
    - Footer (if showActions): Edit button (ghost) and Delete button (ghost, red text) using existing Button component
    - If no photos: show placeholder with "📷 Nessuna foto" centered

    **3. Create `src/screens/Admin/modals/AddDishModal.tsx`**
    ...
    - For new dish: generate a temporary UUID for dishId (needed for photo upload path) — the Edge Function will use it as the actual dish ID if provided as `dishId` in create mode, OR let the EF generate one. Simplest: upload photos with a temp folder name, then pass URLs to EF.
    ...
    **4. Create `src/screens/Admin/tabs/DishesTab.tsx`**
    ...
    - Delete action: opens ConfirmModal (created in 02-01), on confirm calls `supabase.functions.invoke('dish-delete', ...)`
    - After successful delete: call competitionStore.removeDish(dishId), show success Toast
    - Empty state: "Nessun piatto ancora. Aggiungi il primo!" centered with emoji "🍳"
    ...
    **5. Update `src/screens/Admin/AdminScreen.tsx`**
    Replace the DishesTab placeholder with the actual `<DishesTab />` component import. Keep other tab placeholders for Plan 02-03.
  </action>
  <verify>
    Run `npm run build` — zero errors. AdminScreen DishesTab renders dish cards. AddDishModal opens and has all form fields. ConfirmModal renders with title/message/buttons.
  </verify>
  <done>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    4. Click "Aggiungi piatto" — AddDishModal should open as bottom-sheet
    5. Fill: name="Carbonara", chef="Marco", add 1-2 photos from device
    6. Submit — dish should appear as DishCard with photo, name, chef
    7. Click edit on the dish — modal reopens with pre-filled data
    8. Change the name, submit — card updates
    9. Click delete — ConfirmModal appears, confirm — dish removed
