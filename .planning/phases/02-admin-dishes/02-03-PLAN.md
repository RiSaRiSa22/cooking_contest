...
@src/screens/Admin/modals/ConfirmModal.tsx (created in 02-01, reuse for phase/reset confirms)
...
<task type="auto">
  ...
  <action>
    ...
    **Section 2 — Statistiche:**
    - Card grid showing: Partecipanti count, Piatti count, Voti count (from store data, votes need fetch — for now show dishes.length, participants.length; votes count can be fetched or shown as "—" until Phase 3)
    - Use emoji + number + label pattern

    **Section 3 — Fase gara:**
    - Current phase displayed with PhaseBanner-style badge
    - "Avanza fase" button (ember variant):
      - In `preparation`: button text "Avvia votazione 🗳️"
      - In `voting`: button text "Concludi gara 🏆"
      - In `finished`: button disabled, text "Gara conclusa"
    - On click: open ConfirmModal (from 02-02) with appropriate message:
      - preparation->voting: "Vuoi avviare la votazione? I partecipanti non potranno più modificare i piatti."
      - voting->finished: "Vuoi concludere la gara? La classifica finale sarà visibile a tutti."
...
<task type="auto">
  ...
  <action>
    **1. Create `src/components/competition/ParticipantCard.tsx`**
    ...
    - Info: nickname (font-semibold), role badge if admin ("👑 Admin" in gold), join time relative
    - Status badges (right side):
      - Dish status: "🍽️" green check if participant has a dish, gray "—" if not
      - Vote status: "🗳️" green check if participant voted, gray "—" if not (vote data available in Phase 3 — for now show "—")
    ...
    **2. Create `src/screens/Admin/tabs/ParticipantsTab.tsx`**
    ...
    - Empty state: "Nessun partecipante ancora" (shouldn't happen — admin is always a participant)
    ...
    **3. Create `src/screens/Admin/tabs/RankingTab.tsx`**
    ...
    - If phase === 'finished' OR phase === 'voting':
      ...
      - Each item: medal emoji for top 3 (🥇🥈🥉), dish name, vote count (shown as "— voti" until Phase 3 provides actual data)
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    5. Tab "Impostazioni":
       a. Competition code visible with copy button — copy and verify clipboard
       b. QR code image renders
       c. Stats show participant and dish counts
       d. Click "Avvia votazione" — ConfirmModal appears
       e. Confirm — phase changes to "voting", PhaseBanner updates
       f. Click "Concludi gara" — confirm — phase changes to "finished"
