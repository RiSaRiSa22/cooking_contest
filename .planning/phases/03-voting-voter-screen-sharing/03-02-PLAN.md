...
must_haves:
  truths:
    ...
    - "Tab Il mio piatto mostra il piatto del partecipante in read-only con foto extra in voting"
...
# Key existing files
@src/store/voterStore.ts                           # Creato in 03-01 — dishes, myDishId, myVotedDishId
...
<task type="auto">
  ...
  <action>
  **DishDetailSheet.tsx** — Bottom-sheet component riusabile (in `components/dishes/` perche potrebbe essere usato anche altrove):
  ...
  Struttura UI (seguire UI_REFERENCE.md sezione 4 — Overlays e Modali):
  - **Overlay:** `fixed inset-0 z-50 bg-black/50 flex items-end` con click su overlay → chiudi
  ...
  - **Foto hero:** prima foto del piatto, `w-full max-h-[300px] object-cover`. Se nessuna foto, placeholder con emoji 🍽️ su sfondo parchment
  ...
  - **Chef label:**
    - In preparation/voting: "🎭 Cuoco misterioso" (font-body text-sm, color ink-light)
  ...
  **GalleryTab.tsx:**
  ...
  - Ogni cella: foto hero (prima foto del piatto) come background-image `object-cover`, o placeholder emoji 🍽️ se nessuna foto
  - Sotto la foto (overlay in basso): nome piatto in testo bianco con text-shadow su sfondo gradiente semi-trasparente
  - Tap su cella → apre DishDetailSheet con il piatto selezionato
...
<task type="auto">
  ...
  **Logica:**
  ...
  - Trovare il piatto corrispondente in `dishes` (dal voterStore). NOTA: in voting, `dishes_public.participant_id` e null — ma `myDishId` viene dalla EF `vote-read` che lo ottiene dalla tabella `dishes` direttamente.
  - Se `myDishId === null` → empty state per ospite: "Non hai aggiunto un piatto a questa gara" con emoji 👀 e suggerimento
  ...
  **Se il piatto esiste, mostrare:**
  ...
  - Nome cuoco (sempre visibile per il PROPRIO piatto — ma dishes_public lo nasconde! Soluzione: mostrare il nickname dalla session come chef name, oppure semplicemente non mostrare chef_name e mostrare "Il tuo piatto" come label)
  - Sezioni info (ingredienti, ricetta, storia) se presenti — display semplice, non espandibile
  ...
  **Foto extra in voting (DISH-06 / VOTR-04):**
  ...
  - Se `phase === 'preparation'`: mostrare nota "Puoi modificare il piatto dal pannello admin" (il partecipante non admin non puo modificare dal VoterScreen in preparation — deve usare il flusso admin o il proprio accesso)
  - Se `phase === 'finished'`: vista completamente read-only

  **Note:**
  - Il partecipante NON admin in preparation puo modificare il piatto tramite DishesTab admin — VOTR-04 dice "modifica in preparation" ma il VoterScreen non ha il flusso di edit completo (AddDishModal). Per v1, mostrare link testuale "Vai al pannello per modificare" oppure semplicemente la vista read-only. La modifica piatto per non-admin e gia gestita via dish-write EF dalla schermata admin.
  - Semplificare: MyDishTab e read-only in tutte le fasi, con l'aggiunta di "foto extra" in voting.
  </action>
  <verify>
  - `npm run build` compila senza errori
