...
must_haves:
  truths:
    - "Un partecipante in fase voting vede la lista dei piatti come card espandibili (tap togla dettagli + pulsante vota) e puo votarne esattamente 1 (non il proprio)"
...
Purpose: Questo e il core value dell'app — i partecipanti possono finalmente votare in modo anonimo. Senza questa slice, l'app non ha funzionalita di voto.
...
<task type="auto">
  ...
  **vote-read/index.ts:**
  ...
  - Tre query parallele con Promise.all:
    1. Voto corrente del partecipante: `votes WHERE competition_id AND participant_id → maybeSingle()` → `myVotedDishId: string | null`
    2. Piatto del partecipante: `dishes WHERE competition_id AND participant_id → maybeSingle()` → `myDishId: string | null`
    3. Conteggio voti per piatto: query SQL raw o aggregazione — selezionare tutti i voti della competizione, aggregare per dish_id → `voteCounts: Array<{ dish_id: string, count: number }>`
       Per il conteggio usare: `supabase.from('votes').select('dish_id').eq('competition_id', competitionId)` e poi aggregare lato JS (Map dish_id → count). Alternativa: usare `supabase.rpc()` ma e overengineering — aggregare 5-20 righe in JS e perfettamente fine.
  ...
  - vote-cast con dati validi → 200 con vote record
  - vote-cast stesso participantId → 200 (upsert aggiorna dish_id)
  - vote-cast phase !== voting → 400
  - vote-cast proprio piatto → 400
  - vote-read → 200 con myVotedDishId, myDishId, voteCounts
  </verify>
...
<task type="auto">
  ...
  <action>
  **voterStore.ts** — Zustand store (NO persist middleware, pattern identico a competitionStore):
  ```typescript
  interface VoterStore {
    ...
    voteCounts: Map<string, number> // dish_id → numero voti
    isLoading: boolean
    error: string | null
    // setters per ogni campo + reset()
  }
  ```
  Tipo `DishPublicWithPhotos`: importare `DishPublic` da database.types.ts + Photo. NOTA: `DishPublic` ha campi nullable (e una view) — `id: string | null`, `name: string | null`, etc. Per comodita definire il tipo con non-null assertion dopo il fetch (filtrare righe con id null).

  **Importante sulla query dishes_public + photos:**
  Prima tentare `supabase.from('dishes_public').select('*, photos(*)').eq('competition_id', competitionId)`. Se funziona, usare direttamente. Se non funziona (PostgREST non risolve la FK sulla view), piano B: due query separate — (1) dishes_public, (2) photos WHERE dish_id IN (...) — e join manuale in JS. La relazione `photos.dish_id → dishes_public.id` esiste nei types (`Relationships` di photos include `dishes_public`), quindi dovrebbe funzionare. Testare al primo run.

  **useVoterData.ts** — Hook che:
  ...
  **VoterScreen.tsx** — Mirror esatto di AdminScreen.tsx:
  ...
  **router.tsx** — Sostituire il placeholder `/voter/:code` con:
  ```tsx
  ...
  **HomeScreen.tsx** — Aggiungere gestione `mode=vote` nel deep link handler:
  ```typescript
  } else if (mode === 'vote') {
  ...
  - Navigare a `/#/voter/CODICE` → VoterScreen con header, PhaseBanner, tab bar (3 tab se non finished)
  - voterStore carica dati da dishes_public e vote-read EF
  - `/#/?code=CODICE&mode=vote` → apre JoinCompModal
  </verify>
...
<task type="auto">
  <name>Task 3: VoteTab — lista piatti votabili con azione voto</name>
  ...
  **1. Status bar in cima** (VOTR-01):
  ...
  - Se phase === 'voting' e ha votato: "Hai votato: [nome piatto] ✅" con sfondo verde chiaro
  ...
  **2. Lista piatti votabili** (solo in phase === 'voting'):
  - Ogni piatto come card espandibile (VOTR-01): stato collapsed mostra foto hero (prima foto), nome piatto, "🎭 Cuoco misterioso". Tap sulla card togga expanded/collapsed. In stato expanded mostra anche ingredienti e ricetta (se presenti) + pulsante "Vota questo piatto".
  - Il proprio piatto (dish.id === myDishId dal voterStore) e visibile ma con overlay "Il tuo piatto" e non cliccabile/espandibile
  - Il piatto votato ha bordo ember e checkmark (visibile anche in collapsed)
  - Pulsante "Vota" nella card expanded → chiama `supabase.functions.invoke('vote-cast', { body: { competitionId, participantId, dishId } })`
  - Dopo voto OK → aggiorna `voterStore.myVotedDishId`
  ...
  **Note di implementazione:**
  - `myDishId` puo essere `null` (ospite senza piatto) → nessun piatto disabilitato
  ...
  - Click su piatto → voto registrato, card mostra checkmark
  - Click su altro piatto → voto cambiato
  - In fase preparation/finished → messaggio appropriato
  </verify>
